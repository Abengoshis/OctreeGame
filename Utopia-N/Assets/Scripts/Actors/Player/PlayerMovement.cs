...
public class PlayerMovement : MonoBehaviour
{
	...
	private void Update()
	{
		if (!oob)
		{
			...
			Vector2 ellipseCoord = new Vector2(((SimulatedCursor.cursorPosition.x - 0.5f) * Screen.height / Screen.width), SimulatedCursor.cursorPosition.y - 0.5f);
	...
	private void FixedUpdate()
	{
		if (!oob)
		{
			...
			model.transform.rotation = CalculateModelRotation();
		...
		else
		{
			...
			else
			{
				...
				model.transform.rotation = Quaternion.Lerp (model.transform.rotation, CalculateModelRotation(), lerp);
			}
	...
	private Quaternion CalculateModelRotation()
	{
		return Quaternion.LookRotation(Camera.main.ViewportToWorldPoint(new Vector3(SimulatedCursor.cursorPosition.x,
		                                                                            SimulatedCursor.cursorPosition.y, 1000.0f)) -
		                               									transform.position, transform.up) *	Quaternion.Euler(0, 0, -angularVelocity.y * 0.5f);
	}
}
